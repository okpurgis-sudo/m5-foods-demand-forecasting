...
この結果から、過去販売数だけでなく、価格・イベント・SNAP・部門情報も予測に使用しました。
...
`middle` / `bottom` グループでは販売数が少なく、販売数が0の日も多いため予測難易度が高くなります。
...
Linux 上でも同じ処理を実行できるようにするための Python スクリプトです。
...
価格・イベント・SNAP・部門情報を追加した特徴量セットで、最終的に最も良い評価結果となりました。
...
### 5. 再実行可能なPythonスクリプト作成

Notebookで検証した処理を、Pythonスクリプトとして実行できるようにしました。

Linux環境では、`scripts/run_linux.sh` からパイプラインを実行できます。
...
* 商品ごとの累計販売数に基づく `top` / `middle` / `bottom` 分類
...
また、販売傾向の分析、再実行可能なPythonスクリプト、Flask API、HTML デモ、AWS S3 / EC2 連携を実装しました。
