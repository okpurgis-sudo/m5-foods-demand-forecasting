...
本プロジェクトでは、機械学習モデルの構築だけでなく、FOODSカテゴリ商品の販売傾向を分析しました。

需要予測モデルを作成する前段階として、商品ごとの販売規模、0販売率、イベント・SNAP・価格による販売数の違いを確認しています。

### Sales Trend by Group

商品を累計販売数に基づいて `top` / `middle` / `bottom` の3グループに分け、日別販売数の推移を確認しました。

![Sales Trend by Group](images/business_sales_trend_by_group.png)

### Zero Sales Rate

販売数が0の日の割合をグループ別に確認しました。  
特に `bottom` グループでは0販売の日が多く、予測難易度が高いことが分かります。

![Zero Sales Rate by Group](images/business_zero_rate_by_group.png)
...
SNAPは地域ごとに対象日が異なるため、州ごとの販売データと組み合わせて分析しました。

![SNAP Impact](images/business_snap_impact.png)

### Price vs Sales

直近期間の販売データと価格データを結合し、価格帯ごとの平均販売数を確認しました。  
価格は需要に影響する要素の一つであるため、モデルの特徴量として使用しました。

![Price vs Sales](images/business_price_vs_sales.png)
...
- `bottom` グループの商品は0販売の日が多く、予測難易度が高い
- イベントやSNAP対象日は販売数に影響する可能性がある
- 価格情報は需要予測において重要な説明変数になり得る
- 販売規模ごとに需要の特徴が異なるため、グループ別にモデルを作成する方針は妥当である

この分析により、単にモデルを構築するだけでなく、需要予測を業務改善や在庫計画に活用する観点も確認しました。
...
* FOODSカテゴリ商品の抽出
* 商品ごとの累計販売数による `top` / `middle` / `bottom` 分類
* 販売規模別の売上推移の可視化
* 0販売率の集計
* イベント日と通常日の平均販売数比較
...
この分析により、モデル構築前に販売傾向を確認し、なぜ販売規模別にモデルを分けたのか、なぜイベント・SNAP・価格を特徴量に入れたのかを説明しやすくしています。
